...
def load_training_data_CNN(train_path, y_val):
    ...
    for i in range(s):
        sys.stdout.write(
            '\r  >> Loading training data from %s (%d/%d)' % (train_path, i+1, s))
        sys.stdout.flush()
...
if __name__ == "__main__":
    ...
    [train_x1, train_y1] = load_training_data_CNN(
        '/home/madhan/Documents/skalenow/face_reco/train_faces/madhan', 0)
    [train_x2, train_y2] = [], []
    # [train_x1, train_y1] = load_training_data_CNN('train_faces/0', 0)
    # [train_x2, train_y2] = load_training_data_CNN('train_faces/1', 1)
    [train_x3, train_y3] = load_training_data_CNN(
        '/home/madhan/Documents/skalenow/face_reco/train_faces/sachin', 1)
    [train_x4, train_y4] = load_training_data_CNN(
        '/home/madhan/Documents/skalenow/face_reco/train_faces/sanath', 2)
    ...
    for step_i in range(step + 1):
        ...
        for sub_step in range(0, data_len, batch_size):
            ...
            if step_i % 10 == 0:
                ...
                cost += ret_cost
                ret_y = sess.run(predict_op, feed_dict={
                                 X: tr_x, Y: tr_y, p_keep_con: 1, p_keep_hidden: 1})
                predict_y = np.append(predict_y, ret_y)
            else:
                sess.run(train_op, feed_dict={
                         X: tr_x, Y: tr_y, p_keep_con: p_conv, p_keep_hidden: p_hidden})
